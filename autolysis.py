...
def narrate_story(analysis, charts, output_dir):
    ...
    messages = {
        ...
        "messages": [
            {"role": "system", "content": "You are a Markdown writer."},  # System message to instruct LLM
            {"role": "user", "content": prompt}  # User message with the prompt
        ],
    }
    story = send_to_llm(messages)  # Get the story generated by the LLM
    with open(os.path.join(output_dir, "README.md"), "w") as file:  # Write the story to a README file
        file.write(story)
...
def main():
    ...
    dataset_path = sys.argv[1]  # Get the dataset path from command line argument
    output_dir = os.path.splitext(os.path.basename(dataset_path))[0]  # Generate output directory name from dataset filename

    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)

    try:
        df = read_csv_file(dataset_path)  # Read the dataset
        analysis = analyze_data(df)  # Analyze the data
        charts = generate_visualizations(df, output_dir)  # Generate visualizations
        narrate_story(analysis, charts, output_dir)  # Create the narrative and save to README.md
        print(f"Analysis complete. See the '{output_dir}' directory for results.")  # Print completion message
